...
const generateCertificate = async (status, userName, courseName, date, year) => {
  try {
    ...
    // Определение пути к изображению в зависимости от статуса
    if (status === 3) {
    ...
    // Возвращаем promise с буфером для потока
    return new Promise((resolve, reject) => {
      pdf.create(htmlWithStyles, options).toBuffer((err, buffer) => {
        if (err) {
          return reject(err);
        }
        resolve(buffer); // Возвращаем буфер для передачи через HTTP-ответ
      });
    });
  } catch (error) {
