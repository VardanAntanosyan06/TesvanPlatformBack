...
const updateCourse = async (req, res) => {
  try {
    ...
    if (!Array.isArray(levelDescriptions_ru)) {
      levelDescriptions_ru = [levelDescriptions_ru];
    } else {
      levelDescriptions_ru = JSON.parse(levelDescriptions_ru);
    }

    if (!Array.isArray(levelDescriptions_am)) {
      levelDescriptions_am = [levelDescriptions_am];
    } else {
      levelDescriptions_am = JSON.parse(levelDescriptions_am);
    }

    await levelDescription.destroy({ where: { courseId } });
    levelDescriptions_en.forEach(async (e, i) => {
      const title_en = e?.title;
      const description_en = e?.description;
      const title_ru = levelDescriptions_ru[i]?.title;
      const description_ru = levelDescriptions_ru[i]?.description;
      const title_am = levelDescriptions_am[i]?.title;
      const description_am = levelDescriptions_am[i]?.description;
