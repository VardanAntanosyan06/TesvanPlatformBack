      ...
      let course = await GroupCourses.findOne({
        ...
        include: [
          ...
          },
          {
            ...
            attributes: [[`title_${language}`,'title'], [`description_${language}`,'description']],
          },
          {
            model: Lesson,
            attributes: [
...
const createCourse = async (req, res) => {
  try {
    ...
    await CoursesContents.create({
      ...
      whyThisCourse: !Array.isArray(req.body[`whyThisCourse_${language}`]) ? [req.body[`whyThisCourse_${language}`]]:JSON.parse(req.body[`whyThisCourse_${language}`]),
...
const getCoursesByFilter = async (req, res) => {
  try {
    ...
    Courses = Courses.map((e) => {
      ...
      (e.courseDate =
        moment(new Date(e.endDate)).diff(new Date(e.startDate), "months") > 0
          ? moment(e.endDate).diff(new Date(e.startDate), "months") +
            " " +
            months[language]
          : moment(e.endDate).diff(new Date(e.startDate), "days") +
            " " +
            days[language]),
        (e.price = e.price);
      (e.saledValue =
    ...
    if (order === "popularity")
      Courses = Courses.sort((a, b) => b.bought - a.bought);
    if (order === "newest")
      Courses = Courses.sort((a, b) => b.courseStartDate - a.courseStartDate);
    if (order === "lowToHigh")
...
const getOneGroup = async (req, res) => {
  try {
    ...
    Courses = {
      ...
      sale: discount,
      saledValue:
        price > 0 ? price - Math.round(price * discount) / 100 : price,
    };
    return res.status(200).json(Courses);
  } catch (error) {
    console.log(error);
